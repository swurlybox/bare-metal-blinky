...
static int L3_read_side_info(bs_t *bs, L3_gr_info_t *gr, const uint8_t *hdr)
{
    ...
    {
        gr_count *= 2;
        main_data_begin = (int) get_bits(bs, 9); /* NOTE: int cast */
    ...
    do
    {
        ...
        /* NOTE: uint8_t cast here */
        gr->preflag = (uint8_t) (HDR_TEST_MPEG1(hdr) ? get_bits(bs, 1) : (gr->scalefac_compress >= 500));
...
static void L3_read_scalefactors(uint8_t *scf, uint8_t *ist_pos, const uint8_t *scf_size, const uint8_t *scf_count, bs_t *bitbuf, int scfsi)
{
    ...
    for (i = 0; i < 4 && scf_count[i]; i++, scfsi *= 2)
    {
        ...
        if (scfsi & 8)
        {
            memcpy(scf, ist_pos, (size_t) cnt); /* NOTE: size_t cast */
        } else
        {
            int bits = scf_size[i];
            if (!bits)
            {
                memset(scf, 0, (size_t) cnt); /* NOTE: cast */
                memset(ist_pos, 0, (size_t) cnt); /* NOTE: cast */
            } else
            {
                int max_scf = (scfsi < 0) ? (1 << bits) - 1 : -1;
                for (k = 0; k < cnt; k++)
                {
                    int s = (int) get_bits(bitbuf, bits);   /* NOTE: cast*/
...
#define BSPOS         ((bs_next_ptr - bs->buf)*8 - 24 + bs_sh)
            ...
            do
            {
                ...
                do
                {
                    ...
                    while (leaf < 0)
                    {
                        FLUSH_BITS(w);
                        w = leaf & 7;
                        leaf = codebook[PEEK_BITS(w) - (long unsigned int)(leaf >> 3)];
                    }
                    FLUSH_BITS(leaf >> 8);
                    ...
                    for (j = 0; j < 2; j++, dst++, leaf >>= 4)
                    {
                        ...
                        if (lsb == 15)
                        {
                            lsb += (int) (PEEK_BITS(linbits));
                            FLUSH_BITS(linbits);
                            CHECK_BITS;
                            *dst = one*L3_pow_43(lsb)*((int32_t)bs_cache < 0 ? -1: 1);
                        } else
                        {
                            *dst = g_pow43[(long unsigned int)(16 + lsb) - (16*(bs_cache >> 31))]*one;
                        }
                        FLUSH_BITS(lsb ? 1 : 0);
                    }
                    CHECK_BITS;
    ...
    for (np = 1 - big_val_cnt;; dst += 4)
    {
        ...
        if (!(leaf & 8))
        {
            leaf = codebook_count1[(long unsigned int) (leaf >> 3) + (bs_cache << 4 >> (32 - (leaf & 3)))];
        }
        FLUSH_BITS(leaf & 7);
...
static void L3_reorder(float *grbuf, float *scratch, const uint8_t *sfb)
{
    ...
    memcpy(grbuf, scratch, (unsigned int)(dst - scratch)*sizeof(float));
}
...
static void L3_imdct_gr(float *grbuf, float *overlap, unsigned block_type, unsigned n_long_bands)
{
    ...
    if (n_long_bands)
    {
        L3_imdct36(grbuf, overlap, g_mdct_window[0], (int) n_long_bands);
        grbuf += 18*n_long_bands;
        overlap += 9*n_long_bands;
    }
    if (block_type == SHORT_BLOCK_TYPE)
        L3_imdct_short(grbuf, overlap, (int) (32 - n_long_bands));
    else
        L3_imdct36(grbuf, overlap, g_mdct_window[block_type == STOP_BLOCK_TYPE], (int) (32 - n_long_bands));
}

static void L3_save_reservoir(mp3dec_t *h, mp3dec_scratch_t *s)
{
    int pos = (int)(((unsigned int)(s->bs.pos + 7))/8u);
    int remains = (int)((unsigned int)(s->bs.limit)/8u - (unsigned int)pos);
    ...
    if (remains > 0)
    {
        memmove(h->reserv_buf, s->maindata + pos, (size_t) remains);
    }
    h->reserv = remains;
}
...
static int L3_restore_reservoir(mp3dec_t *h, bs_t *bs, mp3dec_scratch_t *s, int main_data_begin)
{
    ...
    memcpy(s->maindata, h->reserv_buf + MINIMP3_MAX(0, h->reserv - main_data_begin),(size_t) (MINIMP3_MIN(h->reserv, main_data_begin)));
    memcpy(s->maindata + bytes_have, bs->buf + bs->pos/8, (size_t)frame_bytes);
...
static void L3_decode(mp3dec_t *h, mp3dec_scratch_t *s, L3_gr_info_t *gr_info, int nch)
{
    ...
    for (ch = 0; ch < nch; ch++, gr_info++)
    {
        ...
        L3_imdct_gr(s->grbuf[ch], h->mdct_overlap[ch], gr_info->block_type, (unsigned int)n_long_bands);
...
int mp3dec_decode_frame(mp3dec_t *dec, const uint8_t *mp3, int mp3_bytes, mp3d_sample_t *pcm, mp3dec_frame_info_t *info)
{
    ...
    if (!pcm)
    {
        return (int)hdr_frame_samples(hdr);
    }
    ...
    return (int)((unsigned int)success*hdr_frame_samples(dec->header));
}
